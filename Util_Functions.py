...
def unix_timestamp_to_nyc_time(str_unix_timestamp):
    try:
        unix_timestamp = int(str_unix_timestamp)
    except ValueError:
        return "API format error!"

    # converts the timestamp directly to NYC time, EST
    nyc_tz = ZoneInfo("America/New_York")
    local_time = datetime.fromtimestamp(unix_timestamp, tz=nyc_tz)
